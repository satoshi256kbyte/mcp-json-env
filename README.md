...
- `/.devcontainer/devcontainer.json`
  - devcontainer 起動時に `.env` ファイルの環境変数をコンテナへ引き継ぎます
...
> **注意:** `.env` ファイルは `.gitignore` に含まれているため、Git にコミットされません。API キーなどの機密情報を安全に管理できます。
...
- devcontainer が `.env` ファイルの環境変数をコンテナへ受け渡すため、Copilot agent mode / Claude Code / Kiro CLI から同じ値を使えます
