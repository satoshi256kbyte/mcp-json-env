...
- `/.devcontainer/devcontainer.json`
  ...
  - ホストの `~/.aws` / `~/.claude` / `~/.claude.json` / `~/.gitconfig` をバインドマウントし、認証情報をホストと共有します
    - `.gitconfig` はホストの credential helper との競合を防ぐため `~/.gitconfig-host` にマウントし、`[include]` で取り込みつつ `credential.helper` をクリアします
- `/pyproject.toml`
...
- ホストの `~/.gitconfig` を `~/.gitconfig-host` にマウントし、コンテナ直接の `~/.gitconfig` としては使用しません
- `postCreateCommand` でコンテナ用の `~/.gitconfig` を作成し、ホスト設定を `[include]` で取り込みつつ `credential.helper` を空に上書きします
- これにより VS Code の `GIT_ASKPASS` によるクレデンシャル転送が正常に動作します
...
- ホスト Mac の `~/.gitconfig` は `~/.gitconfig-host` にマウントし、コンテナ用の `~/.gitconfig` で `[include]` しつつ credential helper を無効化しています（VS Code のクレデンシャル転送と競合しないようにするため）
